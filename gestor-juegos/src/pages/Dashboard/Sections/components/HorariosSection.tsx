...
import { Horario, Actividad } from "../../../../hooks/useHorarios";
import {
  Plus,
  Calendar,
  Clock,
...
// Función para crear horario (simplificada)
const createHorarioAPI = async (
  horario: Omit<Horario, "_id">
): Promise<Horario> => {
  try {
...
export const HorariosSection: React.FC<HorariosSectionProps> = ({
  ...
  // Define themeClasses basado en el tema actual
  const themeClasses = {
  ...
  // Log para debug
  console.log("🎯 HorariosSection renderizado con:", {
    totalHorarios: data.length,
    horarios: data,
    user: user?.uid,
  });
  ...
  const calcularProgreso = (horario: Horario): number => {
    ...
    Object.values(horario.detalles).forEach((actividades) => {
      (actividades as Actividad[]).forEach((act: Actividad) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Crear horario usando la API de MongoDB
      const horarioCreado = await createHorarioAPI(nuevoHorario);

      if (onHorarioCreated) {
        onHorarioCreated(horarioCreado);
      }
      ...
      alert("Error al crear el horario. Por favor, inténtalo de nuevo.");
    } finally {
      setLoading(false);
    }
  };
                      ...
                      <div className="text-lg font-bold text-purple-400">
                        {horario.detalles
                          ? Object.values(horario.detalles)
                              .flat()
                              .filter((a: any) =>
                                (a as Actividad).descripcion.trim()
                              ).length
                          : 0}
                      </div>
                      <div className={`text-xs ${themeClasses.textSecondary}`}>
